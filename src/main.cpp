...
void process(DataSource data_source, string out_file_name) {
  ...
  for(size_t i = 0; i < size; ++i) {
    ...
    VectorXd estimation = ukf.x_;

    // output the estimation
    out_file << estimation(0) << "\t"; // pos1 estimation
    out_file << estimation(1) << "\t"; // pos2 estimation
    out_file << estimation(2) << "\t"; // vel_abs estimation
    out_file << estimation(3) << "\t"; // yaw_angle estimation
    out_file << estimation(4) << "\t"; // yaw_rate estimation
    ...
    VectorXd x_est = VectorXd(4);
    x_est << ukf.x_(0),                  // x
            ukf.x_(1),                  // y
            ukf.x_(2) * cos(ukf.x_(3)), // vx
            ukf.x_(2) * sin(ukf.x_(3)); // vy

    estimations.push_back(x_est);
